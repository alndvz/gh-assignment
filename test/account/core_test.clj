...
(defn write-many-wait [entities & {:keys [match-entity]}]
  (write-many entities :wait? true
              :match-entity match-entity))

(deftest test-creating-an-account
  (with-redefs [db/write-many write-many-wait]
    (let [new-account (sut/create account-name)
      ...
      (is (string? (:account-number new-account))
